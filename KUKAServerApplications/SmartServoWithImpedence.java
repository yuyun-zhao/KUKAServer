...
/**
 * This example: Null space motion with torque feedback for obstacle navigation
 * during contact.
 * 
 */
...
public class SmartServoWithImpedence {
	...
	public static double[] getControlParameters(String s) {
		double[] val = null;
		String thestring = s;
		...
		if (st.hasMoreTokens()) {
			...
			while (st.hasMoreTokens()) {
				...
				if (j < 7) {
					...
					val[j] = Double.parseDouble(stringData);
				}

				j++;
			}
			// MatlabToolboxServer.daCommand="";
//			MatlabToolboxServer.command.delete(0,
//					MatlabToolboxServer.command.length());
	...
	public static void startRealTimeWithImpedence(LBR lbr, Controller k,
			double massOfTool, double[] toolsCOMCoordiantes, double cStifness,
		...
		_lbr = lbr;
		MASS = massOfTool;
		CENTER_OF_MASS_IN_MILLIMETER[0] = toolsCOMCoordiantes[0];

		CENTER_OF_MASS_IN_MILLIMETER[1] = toolsCOMCoordiantes[1];

		CENTER_OF_MASS_IN_MILLIMETER[2] = toolsCOMCoordiantes[2];
		...
		_loadData.setCenterOfMass(CENTER_OF_MASS_IN_MILLIMETER[0],
		...
		ObjectFrame aTransformation = _toolAttachedToLBR.addChildFrame(
	...
	protected static CartesianImpedanceControlMode createCartImp(
			double cStifness, double rStifness, double nStifness) {
	...
	public static void startImpedenceControl(double cStifness,
		...
		final CartesianImpedanceControlMode cartImp = createCartImp(cStifness,
				rStifness, nStifness);
